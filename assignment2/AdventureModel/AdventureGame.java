...
public class AdventureGame implements Serializable {
    ...
    private final String[] actionVerbs = {"QUIT","INVENTORY","TAKE","DROP","HEAL"}; //List of action verbs (other than motions) that exist in all games. Motion vary depending on the room and game.
    ...
    public boolean movePlayer(String direction) {
        ...
        for (Passage entry : possibilities) {
            ...
            System.out.println(entry.getDirection()+" "+ entry.getDestinationRoom());
            if (chosen == null && entry.getIsBlocked() && !Objects.equals(entry.getKeyName(), "")) {

                if (this.player.getInventory().contains(entry.getKeyName())) {
                    chosen = entry; //we can make it through, given our stuff
                    break;
                } else if (entry.getKeyName().equalsIgnoreCase("SportTroll") || entry.getKeyName().equalsIgnoreCase("MathTroll") || entry.getKeyName().equalsIgnoreCase("DeathTroll")|| entry.getKeyName().equalsIgnoreCase("iSpyTroll")) {
    ...
    public String interpretAction(String command){
        ...
        if (motionTable.optionExists(inputArray[0])) {
            ...
            if (!movePlayer(inputArray[0])) {
                int i =0;
                while(i <= this.player.getCurrentRoom().getMotionTable().getDirection().size() - 1){
                    if (this.player.getCurrentRoom().getMotionTable().getDirection().get(i).getKeyName()!= null && this.player.getCurrentRoom().getMotionTable().getDirection().get(i).getKeyName().contains("Troll")){
                        val = true;
                        break;
                    }
                    i++;
                ...
                if (val){
                    if (this.player.getCurrentRoom().getMotionTable().getDirection().get(i).getKeyName().equalsIgnoreCase("SportTroll")){
                        return "SportTroll";
                    } if (this.player.getCurrentRoom().getMotionTable().getDirection().get(i).getKeyName().equalsIgnoreCase("MathTroll")) {
                        return "MathTroll";
                    } if (this.player.getCurrentRoom().getMotionTable().getDirection().get(i).getKeyName().equalsIgnoreCase("DeathTroll")) {
                        return "DeathTroll";
                    }if (this.player.getCurrentRoom().getMotionTable().getDirection().get(i).getKeyName().equalsIgnoreCase("iSpyTroll")) {
                        return "iSpyTroll";
                    }
                }
            ...
            if(inputArray[0].equals("QUIT")) { return "GAME OVER"; } //time to stop!
            ...
            else if(inputArray[0].equalsIgnoreCase("HEAL") && inputArray.length > 1) return "NO SUCH COMMAND";
            else if(inputArray[0].equalsIgnoreCase("HEAL") && inputArray.length == 1){
                if(this.player.heal_player()) {
                    return this.player.username.toUpperCase() + " HAS HEALED!";
                }else {
                    return "YOU HAVE NO HEALTHPACKS OR ARE ALREADY AT 100 HP!";
                }
            }
                ...
                if(this.player.checkIfObjectInInventory(inputArray[1])) {
                    int size = this.player.inventory.size();
                    this.player.dropObject(inputArray[1]);
                    if(this.player.inventory.size() == size){
                        return "YOU CAN NOT DROP A WEAPON!";
                    }else {
