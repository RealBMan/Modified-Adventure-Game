...
public class AdventureLoader {
    ...
    public void parseObjects() throws IOException {
        ...
        while (buff.ready()) {
            String type_objects = buff.readLine();
            if(Objects.equals(type_objects, "Weapon")){
                String objectName = buff.readLine();
                String objectDescription = buff.readLine();
                String objectLocation = buff.readLine();
                int objectdamage = Integer.parseInt(buff.readLine());
                String separator = buff.readLine();
                if (separator != null && !separator.isEmpty())
                    System.out.println("Formatting Error!");
                int i = Integer.parseInt(objectLocation);
                Room location = this.game.getRooms().get(i);
                Weapon object = new Weapon(objectName, objectDescription, location, objectdamage);
                location.addGameObject(object);
            } else if (Objects.equals(type_objects, "HealthPack")) {
                String objectName = buff.readLine();
                String objectDescription = buff.readLine();
                String objectLocation = buff.readLine();
                int objecthp = Integer.parseInt(buff.readLine());
                String separator = buff.readLine();
                if (separator != null && !separator.isEmpty())
                    System.out.println("Formatting Error!");
                int i = Integer.parseInt(objectLocation);
                Room location = this.game.getRooms().get(i);
                HealthPack object = new HealthPack(objectName, objectDescription, location, objecthp);
                location.addGameObject(object);
            }else {
                String objectName = type_objects;
                String objectDescription = buff.readLine();
                String objectLocation = buff.readLine();
                String separator = buff.readLine();
                if (separator != null && !separator.isEmpty())
